      ...
      <div class="space-y-8">
        ...
        <section>
          <div class="flex items-center gap-3 mb-4">
            ...
            <h3 class="text-lg font-bold text-slate-800">Display Preferences</h3>
          </div>
          
          <div class="bg-white rounded-2xl p-6 shadow-sm border border-white">
            <div class="space-y-4">
              <div>
                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-2">Theme</label>
                <div class="flex gap-3">
                  <button 
                    @click="preferences.theme = 'light'"
                    :class="preferences.theme === 'light' ? 'border-sky-500 bg-sky-50' : 'border-slate-200'"
                    class="flex items-center gap-2 px-4 py-2 rounded-xl border-2 transition-all"
                  >
                    <PhSun :size="18" :class="preferences.theme === 'light' ? 'text-sky-500' : 'text-slate-400'" />
                    <span class="text-sm font-medium text-slate-700">Light</span>
                  </button>
                  <button 
                    @click="preferences.theme = 'dark'"
                    :class="preferences.theme === 'dark' ? 'border-sky-500 bg-sky-50' : 'border-slate-200'"
                    class="flex items-center gap-2 px-4 py-2 rounded-xl border-2 transition-all"
                  >
                    <PhMoon :size="18" :class="preferences.theme === 'dark' ? 'text-sky-500' : 'text-slate-400'" />
                    <span class="text-sm font-medium text-slate-700">Dark</span>
                  </button>
                  <button 
                    @click="preferences.theme = 'system'"
                    :class="preferences.theme === 'system' ? 'border-sky-500 bg-sky-50' : 'border-slate-200'"
                    class="flex items-center gap-2 px-4 py-2 rounded-xl border-2 transition-all"
                  >
                    <PhDesktop :size="18" :class="preferences.theme === 'system' ? 'text-sky-500' : 'text-slate-400'" />
              ...
              <div class="border-t border-slate-50 pt-4">
                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-2">Items Per Page</label>
                <select 
                  v-model="preferences.itemsPerPage"
                  class="bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 outline-none focus:border-sky-300"
                >
              ...
              <div class="flex items-center justify-between py-2 border-t border-slate-50">
                <div>
                  <p class="font-medium text-slate-800">Compact Mode</p>
                  <p class="text-xs text-slate-400 mt-0.5">Reduce spacing between elements</p>
                </div>
                <label class="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" v-model="preferences.compactMode" class="sr-only peer">
                  <div class="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-sky-500"></div>
                </label>
              </div>
            </div>
